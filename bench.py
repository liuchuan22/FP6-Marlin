...
def get_problem(m, n, k):
    ...
    B = torch.randint(4294967295, (k*3//32, n*2), device=dev, dtype=torch.uint32).contiguous()
...
def benchmark_quant(A, B, C, s, thread_k, thread_n, sms):
    ...
    res = benchmark(lambda: fp6_marlin.mul(A, B, C, s, workspace, thread_k, thread_n, -1))
    return {
        's': res,
        'TFLOP/s': 2 * A.numel() * C.shape[1] / res / 10 ** 12,
...
for model, layers in MODELS.items():
        ...
        for batch in batchsizes:
            ...
            for layer in layers:
                A, B, C, B_ref, s = get_problem(batch, layer[1], layer[0])
